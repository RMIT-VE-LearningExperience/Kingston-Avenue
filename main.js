...
// SPW piles from 02 — CB1/SPW5+RTW1, bored piers from 04 — CB1/SPW2+3.
function holeCatsForStage(idx) {
  const cats = [];
  if (idx >= 2) cats.push('spw_wall');
  if (idx >= 4) cats.push('piers');
  return cats;
}
...
function buildHoleMarkers() {
  ...
  holeCatsForStage(stageIndex).forEach(cat => {
    ...
    clusters.forEach(c => {
      ...
      disc.position.set(c.x, y + HOLE_THICKNESS / 2, c.z);
      group.add(disc);
    });
...
function loadStage(idx) {
  ...
  loader.load(stage.file, (gltf) => {
    ...
    scene.add(root);
    root.updateMatrixWorld(true);   // hole markers measure world-space bounds
    currentRoot = root;
