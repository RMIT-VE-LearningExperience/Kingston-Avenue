...
// category is carved in any stage whose export contains it (the stage models
// only include piles that exist at that stage), so no manifest-index gating.
const PILE_CATS = ['piers', 'spw_1', 'spw_2', 'spw_3', 'spw_4', 'spw_5', 'spw_wall'];
function holeCatsForStage(idx) {
  return PILE_CATS.filter(cat => (groups[cat] || []).length > 0);
}
