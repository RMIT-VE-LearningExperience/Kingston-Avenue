...
- **Scope view** (`renderScopeInset()`): "Scope Staff 1/2" renders a circular telescope inset (scissor/viewport pass, same technique as the view-gizmo cube) from the instrument's optical axis aimed horizontally at the staff, auto-zoomed to frame ~0.6 m of staff, with a CSS crosshair + stadia reticle. The crosshair sits exactly at sight height so the E-pattern reading can be taken visually; sight lines/markers are hidden during this pass so they don't cover the graduations.
...
- **Setout grid** (`buildSetoutGrid()`, from the boundary hull): drawing A.02's structural grid as dashed lines with circled bubble sprites at both ends, riding the Level Plane. Numbered lines 1/2/3 at 26.88/13.89/5.50 m west of the east boundary (top chain 12990/8390/5500); lettered lines A–E at 21.85/20.14/12.68/7.59/1.00 m north of the setout point (the hull's SE corner; chains 1710/7460/5090/6590/1000). Verified: grid 3 ≈ SPW4 wall line (x 28.8 vs 28.7), grid E ≈ slab south edge (z −16.15 vs −16.08). A/B intentionally float north of the modeled block (they belong to the parcel's wider west part).
