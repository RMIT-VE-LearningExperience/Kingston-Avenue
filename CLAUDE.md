...
- **Line-of-sight readout**: the instrument sights horizontally at `tripod.position.y + instrumentTop` (measured from the GLB at load). Readable iff `staffBase <= sightY <= staffBase + 3`. Staff and level move fully independently — nothing is clamped to keep them in range; students practice differential levelling by discovering the staff is out of range and relocating the level themselves. The dashed sight line is yellow with a crossing marker + live reading while readable, red with an out-of-range hint ("move the level lower/higher") otherwise. Both instruments are kept on the terrain though: `keepOnTerrain` reverts a drag that leaves no ground underneath.
